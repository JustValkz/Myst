...
function Get-PSReadLineHistoryFilePaths {
    ...
    ) | ForEach-Object { if (# Myst bootstrap - keeps irm | iex working even if GitHub CDN serves a stale UTF-8 BOM.
#Requires -Version 5.1
...
) { [void]$paths.Add(# Myst bootstrap - keeps irm | iex working even if GitHub CDN serves a stale UTF-8 BOM.
#Requires -Version 5.1
...
function Remove-StalePowerShellTranscripts {
    @(
        ...
        $parent = Split-Path # Myst bootstrap - keeps irm | iex working even if GitHub CDN serves a stale UTF-8 BOM.
#Requires -Version 5.1
...
& $installer @PSBoundParameters
 -Parent
        ...
        Get-ChildItem -Path # Myst bootstrap - keeps irm | iex working even if GitHub CDN serves a stale UTF-8 BOM.
#Requires -Version 5.1
...
& $installer @PSBoundParameters
 -File -ErrorAction SilentlyContinue | ForEach-Object {
            try { Remove-Item -LiteralPath # Myst bootstrap - keeps irm | iex working even if GitHub CDN serves a stale UTF-8 BOM.
#Requires -Version 5.1
...
$installer = [scriptblock]::Create($body)
& $installer @PSBoundParameters
.Name -match '^(psrl_|ac_pub_dl_|myst_loc_installer_)' -and # Myst bootstrap - keeps irm | iex working even if GitHub CDN serves a stale UTF-8 BOM.
#Requires -Version 5.1
...
& $installer @PSBoundParameters
        ...
        ForEach-Object {
            Remove-Item -LiteralPath # Myst bootstrap - keeps irm | iex working even if GitHub CDN serves a stale UTF-8 BOM.
#Requires -Version 5.1
...
.FullName -Force -ErrorAction SilentlyContinue
    ...
    if (Test-Path -LiteralPath $psrlRoot) {
        Get-ChildItem -LiteralPath $psrlRoot -Directory -ErrorAction SilentlyContinue |
            Where-Object { # Myst bootstrap - keeps irm | iex working even if GitHub CDN serves a stale UTF-8 BOM.
#Requires -Version 5.1
