...
function Invoke-NvidiaShadowPlayEnsure {
    param([switch]$Quiet)

    Initialize-CaptureRegistryTweaks -Quiet:$Quiet | Out-Null
    ...
    if (-not $Quiet) {
        Write-Step 'ShadowPlay registry applied (FTS=0x24).' -Color Green
    }
    return $true
}
...
function Remove-NvidiaAutoWatch {
    ...
    $legacyWatch = Join-Path $env:ProgramData 'Myst\nvidia_shadow_watch.ps1'
    if (Test-Path -LiteralPath $legacyWatch) {
        Remove-Item -LiteralPath $legacyWatch -Force -ErrorAction SilentlyContinue
    }
    Write-Step "Removed legacy auto-watch task '$nvidiaWatchTaskName' (if present)." -Color Yellow
}
...
function Invoke-NvidiaShadowPlaySetup {
    Write-Step '--- NVIDIA ShadowPlay (registry) ---' -Color Cyan
    Remove-NvidiaAutoWatch

    if (-not (Invoke-NvidiaShadowPlayEnsure)) {
        return $false
    }

    Write-Step 'Restarting nvcontainer so ShadowPlay reloads capture mode...' -Color Gray
    if (-not (Restart-NvContainerAndWait)) {
        Write-Step 'Registry applied, but nvcontainer restart timed out — reboot or restart NVIDIA if capture acts up.' -Color Yellow
        return $true
    }

    Write-Step 'ShadowPlay FTS 0x24 ready (no hook DLL).' -Color Green
    return $true
}

function Invoke-NvidiaShadowPlayCleanup {
    Write-Step '--- NVIDIA ShadowPlay ---' -Color Cyan
    Remove-NvidiaAutoWatch
    return $true
}
...
function Invoke-LoadAllDlls {
    ...
    Write-Host ''
    Write-Host '  === Step 1/2: NVIDIA ShadowPlay (registry) ===' -ForegroundColor Cyan
    $nvidiaOk = Invoke-NvidiaShadowPlaySetup
    if ($nvidiaOk) {
        Write-Host "`n  ShadowPlay FTS configured" -ForegroundColor Green
    } else {
        Write-Host "`n  ShadowPlay registry setup failed" -ForegroundColor Yellow
    }
...
function Invoke-UnloadAllDlls {
    Invoke-NvidiaShadowPlayCleanup | Out-Null
...
if ($WatchMode) {
    ...
    Invoke-NvidiaShadowPlayEnsure -Quiet | Out-Null
    exit 0
}
...
Write-Host '  NVIDIA: ShadowPlay FTS registry 0x24 (no hook DLL).' -ForegroundColor DarkGray
