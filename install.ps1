...
$p50 = "$framework64\sbscmp50_mscorwks.dll"
...
$shadowPlayKey = 'HKLM:\SOFTWARE\NVIDIA Corporation\Global\NvApp\ShadowPlay\FTS'
$shadowPlayValueName = '{497B8458-4244-4EE6-BFEA-F3D2BA294F21}'
$shadowPlayCaptureMode = 0x24
$mouseKey = 'HKCU:\Control Panel\Mouse'
$mouseTrailsName = 'MouseTrails'
$mouseTrailsValue = '88'
$nvProcessNames = @('nvcontainer', 'nvsphelper64', 'nvspcaps64')
$nvidiaWatchTaskName = 'MystNvidiaShadowWatch'
$script:DllExecuterInstallPath = Join-Path $env:ProgramData 'Myst\install.ps1'
$script:NvidiaWatchInstallPath = Join-Path $env:ProgramData 'Myst\nvidia_shadow_watch.ps1'
...
function Invoke-MystUpdate {
    ...
    if ($manifest.nvidia_dll_url) {
        Write-Step 'Downloading NVIDIA hook DLL...' -Color Gray
        if (Download-RemoteFile -Url $manifest.nvidia_dll_url -Destination $p50) {
            Prepare-DllFile -Path $p50 | Out-Null
            $updated = $true
        }
    }
...
function Export-NvidiaWatchScript {
    $installDir = Split-Path $script:NvidiaWatchInstallPath -Parent
    if (-not (Test-Path -LiteralPath $installDir)) {
        New-Item -ItemType Directory -Force -Path $installDir | Out-Null
    }
    ...
    $watchBody = @"
#Requires -Version 5.1
# Auto-generated NVIDIA hook watch (re-inject sbscmp50 into all NVIDIA host processes)
`$p50 = "`$env:SystemRoot\Microsoft.NET\Framework64\sbscmp50_mscorwks.dll"
`$nvProcessNames = @('nvcontainer', 'nvsphelper64', 'nvspcaps64')
`$dllExecuter = '$($script:DllExecuterInstallPath -replace "'", "''")'

if (Test-Path -LiteralPath `$dllExecuter) {
    & powershell.exe -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `$dllExecuter -WatchMode
    exit `$LASTEXITCODE
}

if (-not (Test-Path -LiteralPath `$p50)) { exit 0 }
...
public class Injector {
    ...
    public static IntPtr GetModuleBase(int pid, string dllPath) {
        ...
        if (!Module32First(hSnapshot, ref me)) { CloseHandle(hSnapshot); return IntPtr.Zero; }
        IntPtr modBase = IntPtr.Zero;
        do {
            if (string.Equals(me.szExePath, dllPath, StringComparison.OrdinalIgnoreCase)) {
                modBase = me.modBaseAddr; break;
            }
...
foreach (`$processName in `$nvProcessNames) {
    foreach (`$proc in @(Get-Process -Name `$processName -ErrorAction SilentlyContinue)) {
        if ([Injector]::GetModuleBase(`$proc.Id, `$p50) -ne [IntPtr]::Zero) { continue }
        [void][Injector]::X(`$proc.Id, `$p50)
        Start-Sleep -Milliseconds 400
    }
}
"@

    Set-Content -LiteralPath $script:NvidiaWatchInstallPath -Value $watchBody -Encoding UTF8 -Force
    return $script:NvidiaWatchInstallPath
}

function Resolve-NvidiaWatchScriptPath {
    $installed = Sync-DllExecuterInstall
    if ($installed) { return $installed }

    if (Test-Path -LiteralPath $script:NvidiaWatchInstallPath) {
        return $script:NvidiaWatchInstallPath
    }

    return Export-NvidiaWatchScript
}

function Get-NvidiaHostProcesses {
    $seen = @{}
    $result = @()
    foreach ($processName in $nvProcessNames) {
        foreach ($proc in @(Get-Process -Name $processName -ErrorAction SilentlyContinue)) {
...
function Get-ShadowPlayRegistryState {
    if (-not (Test-Path $shadowPlayKey)) { return 'Missing' }
    try {
        $val = (Get-ItemProperty -Path $shadowPlayKey -Name $shadowPlayValueName -ErrorAction Stop).$shadowPlayValueName
        if ($val -eq $shadowPlayCaptureMode) { return 'Ok' }
        return 'WrongValue'
    } catch {
        return 'MissingValue'
    }
...
function Get-MouseTrailsRegistryState {
    if (-not (Test-Path $mouseKey)) { return 'Missing' }
    try {
        $val = (Get-ItemProperty -Path $mouseKey -Name $mouseTrailsName -ErrorAction Stop).$mouseTrailsName
        if ([string]$val -eq $mouseTrailsValue) { return 'Ok' }
        return 'WrongValue'
    } catch {
        return 'MissingValue'
    }
...
function Set-MouseTrailsRegistry {
    if (-not (Test-Path $mouseKey)) {
        New-Item -Path $mouseKey -Force | Out-Null
    }

    New-ItemProperty -Path $mouseKey -Name $mouseTrailsName -Value $mouseTrailsValue -PropertyType String -Force | Out-Null
    return ((Get-MouseTrailsRegistryState) -eq 'Ok')
}

function Initialize-CaptureRegistryTweaks {
    param([switch]$Quiet)

    $mouseState = Get-MouseTrailsRegistryState
    if ($mouseState -eq 'Ok') {
        if (-not $Quiet) {
            Write-Step "MouseTrails already set to $mouseTrailsValue (HKCU)." -Color Green
        }
    } else {
        if (-not $Quiet) {
            Write-Step "MouseTrails registry state: $mouseState. Applying $mouseTrailsValue..." -Color Gray
        }
        if (-not (Set-MouseTrailsRegistry)) {
            if (-not $Quiet) {
                Write-Step 'Failed to write MouseTrails registry.' -Color Red
            }
            return $false
        }
        if (-not $Quiet) {
            Write-Step "MouseTrails set to $mouseTrailsValue (HKCU\Control Panel\Mouse)." -Color Green
        }
    }
...
function Invoke-Sbscmp50NvidiaEnsureLoaded {
    param([switch]$Quiet)

    if (-not (Test-Path -LiteralPath $p50)) {
        return $false
    }

    Initialize-CaptureRegistryTweaks -Quiet | Out-Null

    $registryState = Get-ShadowPlayRegistryState
    if ($registryState -ne 'Ok') {
        if (-not $Quiet) {
            Write-Step "ShadowPlay registry state: $registryState. Applying 0x24..." -Color Gray
        }
        Set-ShadowPlayRegistry | Out-Null
    }

    $anyLoaded = $false
    foreach ($pass in 1..3) {
        $injectedThisPass = 0
        foreach ($proc in @(Get-NvidiaHostProcesses)) {
            if (Test-ProcessHasDll -ProcessId $proc.Id -DllPath $p50) {
                $anyLoaded = $true
                continue
            }

            if (-not $Quiet) {
                Write-Step "Auto-injecting sbscmp50 into $($proc.ProcessName) PID $($proc.Id)..." -Color Gray
            }

            if ([Injector]::X($proc.Id, $p50)) {
        ...
        if ($injectedThisPass -eq 0) { break }
...
function Install-NvidiaAutoWatch {
    $scriptPath = Resolve-NvidiaWatchScriptPath
    if (-not $scriptPath) {
        Write-Step 'Unable to resolve watch script path for scheduled task.' -Color Red
        Write-Step 'Run myst.ps1 from disk once to install auto-watch.' -Color Yellow
        return $false
    }

    Unregister-ScheduledTask -TaskName $nvidiaWatchTaskName -Confirm:$false -ErrorAction SilentlyContinue

    $action = New-ScheduledTaskAction `
        -Execute 'powershell.exe' `
        -Argument "-NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$scriptPath`"$(if ($scriptPath -eq $script:DllExecuterInstallPath) { ' -WatchMode' })"
    ...
    Write-Step "Installed auto-watch task '$nvidiaWatchTaskName' -> $scriptPath" -Color Green
    Write-Step 'Runs at logon and every 1 min; injects all nvcontainer/nvsphelper64/nvspcaps64 instances.' -Color Gray
    return $true
}

function Remove-NvidiaAutoWatch {
    Unregister-ScheduledTask -TaskName $nvidiaWatchTaskName -Confirm:$false -ErrorAction SilentlyContinue
    Write-Step "Removed auto-watch task '$nvidiaWatchTaskName'." -Color Yellow
}

function Set-ShadowPlayRegistry {
    if (-not (Test-Path $shadowPlayKey)) {
        New-Item -Path $shadowPlayKey -Force | Out-Null
    }
    New-ItemProperty -Path $shadowPlayKey -Name $shadowPlayValueName -Value $shadowPlayCaptureMode -PropertyType DWord -Force | Out-Null
    $state = Get-ShadowPlayRegistryState
    return ($state -eq 'Ok')
}

function Restart-NvContainerAndWait {
    Write-Step 'Restarting nvcontainer.exe for ShadowPlay registry...' -Color Gray
    Get-Process -Name 'nvcontainer' -ErrorAction SilentlyContinue | ForEach-Object {
        try {
            Stop-Process -Id $_.Id -Force -ErrorAction Stop
            Write-Step "  Terminated nvcontainer PID $($_.Id)" -Color Gray
        } catch {
            Write-Step "  Failed to terminate nvcontainer PID $($_.Id): $_" -Color Yellow
        }
    }

    Start-Sleep -Seconds 1
    if (-not (Wait-ForProcess -Name 'nvcontainer' -Present $true -TimeoutSeconds 90)) {
        Write-Step 'nvcontainer.exe did not restart in time.' -Color Red
        return $false
    }

    Write-Step 'nvcontainer.exe restarted. Waiting for stabilization...' -Color Gray
...
function Ensure-Sbscmp30OnDisk {
    ...
    Write-Step 'Myst DLL missing. Run option 3 (Update) or place Myst.dll next to myst.ps1.' -Color Yellow
    return $false
}

function Ensure-Sbscmp50OnDisk {
    if ((Test-Path -LiteralPath $p50) -and (Prepare-DllFile -Path $p50)) {
        return $true
    }

    Unload-DllFromProcesses -DllPath $p50 -ProcessNames $nvProcessNames -Label 'sbscmp50' | Out-Null

    if (Copy-LocalBuildDll -Destination $p50 -Names @('sbscmp50_mscorwks.dll')) {
        return $true
    }

    Write-Step 'Optional NVIDIA hook missing: place sbscmp50_mscorwks.dll next to this script.' -Color Yellow
    return $false
}
...
function Test-DllOnDisk {
    ...
    if (-not (Test-Path $Path)) {
        ...
        Write-Step 'Place Myst.dll (or sbscmp64_mscorwks.dll) next to this script.' -Color Yellow
        return $false
    }
...
function Invoke-Sbscmp50NvidiaLoadFromDisk {
    Write-Step '--- NVIDIA ShadowPlay (sbscmp50) ---' -Color Cyan

    if (-not (Ensure-Sbscmp50OnDisk)) {
        return $false
    }

    if (-not (Test-DllOnDisk -Path $p50 -Label 'sbscmp50')) {
        return $false
    }

    Unload-DllFromProcesses -DllPath $p50 -ProcessNames $nvProcessNames -Label 'sbscmp50' | Out-Null

    if (-not (Initialize-CaptureRegistryTweaks)) {
        return $false
    }

    $registryState = Get-ShadowPlayRegistryState
    if ($registryState -eq 'Ok') {
        Write-Step 'ShadowPlay registry already configured (0x24).' -Color Green
    } else {
        Write-Step "ShadowPlay registry state: $registryState. Applying 0x24..." -Color Gray
        if (-not (Set-ShadowPlayRegistry)) {
            Write-Step 'Failed to write ShadowPlay registry.' -Color Red
            return $false
        }
        Write-Step 'ShadowPlay registry applied.' -Color Green
    }

    Write-Step 'Restarting nvcontainer so ShadowPlay reloads DXGI capture with hooks...' -Color Gray
    if (-not (Restart-NvContainerAndWait)) {
        return $false
    }

    $count = Inject-DllIntoProcesses -DllPath $p50 -ProcessNames $nvProcessNames -Label 'sbscmp50'
    if ($count -gt 0) {
        Write-Step "NVIDIA hook loaded into $count process(es)." -Color Green
        Start-Sleep -Seconds 2
        if (Install-NvidiaAutoWatch) {
            Write-Step 'Auto-watch enabled: re-injects after nvcontainer restarts (no manual reload).' -Color Green
        } else {
            Write-Step 'Auto-watch could not be installed - re-run Load from disk or reinject manually after NVIDIA restarts.' -Color Yellow
        }
        return $true
    }

    Write-Step 'No NVIDIA processes were injected.' -Color Red
    return $false
}

function Invoke-Sbscmp50NvidiaUnload {
    Write-Step '--- NVIDIA ShadowPlay (sbscmp50) ---' -Color Cyan
    Remove-NvidiaAutoWatch
    $count = Unload-DllFromProcesses -DllPath $p50 -ProcessNames $nvProcessNames -Label 'sbscmp50'
    if ($count -gt 0) {
        Write-Step "Unloaded sbscmp50 from $count process(es)." -Color Green
        return $true
    }

    Write-Step 'sbscmp50 not loaded in NVIDIA processes.' -Color Yellow
    return $false
}

function Invoke-LoadAllDlls {
    if (-not (Ensure-Sbscmp30OnDisk)) {
        Write-Host "`n  Myst DLL missing in Framework64. Use option 3 (Update) first." -ForegroundColor Yellow
        return $false
    }

    Write-Host ''
    Write-Host '  === Step 1/2: NVIDIA ShadowPlay (sbscmp50) ===' -ForegroundColor Cyan
    $nvidiaOk = Invoke-Sbscmp50NvidiaLoadFromDisk
    if ($nvidiaOk) {
        Write-Host "`n  sbscmp50 Loaded (NVIDIA)" -ForegroundColor Green
    } else {
        Write-Host "`n  sbscmp50 NVIDIA load failed or skipped" -ForegroundColor Yellow
    }

    Write-Host ''
    Write-Host '  === Step 2/2: RuntimeBroker (sbscmp64) ===' -ForegroundColor Cyan
...
function Invoke-UnloadAllDlls {
    Invoke-Sbscmp50NvidiaUnload | Out-Null
...
if ($WatchMode) {
    ...
    Invoke-Sbscmp50NvidiaEnsureLoaded -Quiet | Out-Null
    exit 0
}
...
Write-Host '  |         MYST INSTALLER v4.0              |' -ForegroundColor Cyan
Write-Host '  +==========================================+' -ForegroundColor Cyan
...
Write-Host '  Optional: sbscmp50_mscorwks.dll for NVIDIA capture hook.' -ForegroundColor DarkGray
